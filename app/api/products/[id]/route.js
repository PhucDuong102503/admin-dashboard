...
export async function PUT(req, context) {
  try {
    ...
    let tensanpham = "", giasanpham = 0, motasanpham = "", idloaisanpham = "", sizesRaw = [];
    let hinhanhsanpham = "";
    ...
    return NextResponse.json({ error: "Lưu thất bại", details: error.message }, { status: 500 });
  }
}
...
export async function DELETE(req, context) {
  try {
    ...
    if (rows.length > 0) {
      const filePath = path.join(process.cwd(), "public", rows[0].hinhanhsanpham || "");
    ...
    return NextResponse.json({ error: "Không thể xóa sản phẩm", details: error.message }, { status: 500 });
  }
}