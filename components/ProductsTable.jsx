...
const ProductsTable = () => {
  ...
  const handleAddProduct = async () => {
    ...
    try {
      ...
      formData.append("giasanpham", Number(giasanpham));
      formData.append("motasanpham", motasanpham);
      formData.append("idloaisanpham", Number(idloaisanpham));
      formData.append("hinhanhsanpham", hinhanhsanpham);
      ...
      if (res.ok) {
        setProducts((prev) => [
          ...prev,
          {
            ...
            sales: 0,
            tenloaisanpham:
              categories.find((c) => c.id === Number(idloaisanpham))?.name ||
              "Không xác định",
          },
        ]);
    ...
    >
        ...
        <div className="bg-[#2f2f2f] p-4 rounded-lg mb-6 space-y-4">
          ...
          <div className="flex flex-col">
            ...
            <select
              ...
              onChange={(e) =>
                setNewProduct({ ...newProduct, idloaisanpham: e.target.value })
              }
              className="bg-[#1e1e1e] text-white border border-gray-600 rounded px-3 py-2 text-sm"
            >
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-700">
          <thead>
            <tr>
              {["Tên", "Loại", "Giá", "Tồn kho", "Ảnh", "Thao tác"].map(
                (header) => (
                  <th
                    key={header}
              ...
              >
                ...
                {editableFields.map((field) => {
                  ...
                  return (
                    <td
                      key={field}
                      className="px-3 md:px-6 py-3 text-sm text-gray-300"
                    >
                      {isEditing ? (
                        <input
